# GUM Explorer — Presenter Notes

Top-to-bottom walkthrough of the left-panel controls. Each entry: what it is,
what it does to the model, what visibly changes on the page, and a line you can
say out loud. Written to be scannable mid-sentence when the train of thought
leaves the station without you.

**The 30-second frame for the whole app:** every control on the left is a term
in the realized-output functional. The right side shows what that term is doing
to you right now. Reality is normalized so that a merely *healthy* organization
produces exactly 1.000 — everything else is measured in units of "how many
healthy orgs of damage is this decision worth."

---

## Session Controls

*(Meta-controls: how the demo behaves, not what the org does.)*
...
- **Say:** "Freeze is the difference between *forecasting* the organization and
  *becoming* it. I'll leave it on; we've all seen what happens otherwise."
...
- **On screen:** The four DORA rows get an amber edge and tint; everything
  ...
  55% opacity.
- **Say:** "This is what your metrics vendor can see. Note that the dimmed
  rows are still *in the equation* — dimming a variable does not remove it
  from your organization. It only removes it from your dashboard."
...
- **Say:** "The model has two postures: the one for the methods section and
...
- **Say:** "Conservative mode believes your stated deploy frequency.
...
- **Model effect:** None.
- **Say (if it comes up):** "Notifications, like urgency, are configurable in
...
- **Say:** "Pin Baseline is for before-and-after experiments: pin the org you
...
- **Elite Performer** — "High trust, low chaos." Note U_eff ≤ policy U: a
  healthy attenuation state *dampens* imposed urgency slightly. **Say:** "In a
  coherent organization, a deadline is just information."
- **High Performer** — the calibration anchor; loads by default. **Say:** "We
  normalized reality to the healthiest org we could responsibly imagine.
...
- **Under Pressure** — the thesis demo. Throughput bar reads *exactly* 1.00×,
  ...
  terms. **Say:** "Same pipeline. Same deploy cadence. The dashboards agree
...
- **Legacy Bog** — debt-dominant but *calm* (low U, low E). Lead time of 48
  hours does the strangling; TDR 0.7 taxes what's left. **Say:** "Nobody here
  is panicking. That is the only nice thing the model has to say about it."
- **Incident Loop** — CFR 0.45 under U 4.0 puts an exponent of ~39 on the
  failure term: (0.55)^39 ≈ 10⁻¹¹. **Say:** "A 45% change-failure rate is
  survivable. A 45% change-failure rate *under maximum urgency* is an
  extinction event. Urgency doesn't change your failure rate — it changes
  what your failure rate costs."
- **Max Entropy** — subtitle "The literature review." Let the 1.3 × 10⁻²⁶ sit
  in silence for a beat. **Say:** nothing. The number is the joke.
...
- **Say:** "Shipping more is genuinely good — it's in the numerator. It is
...
- **Say:** "Lead time is friction. It has no redeeming exponent, no clever
...
- **Say:** "Failure rate is not a cost. It's a *base*. Management urgency is
...
- **Say:** "The model doesn't distinguish time spent shipping from time spent
...
- **On screen:** Morale bar; the M sweep tab draws a dashed "M=1 threshold"
  ...
  touching the slider.
- **Say:** "Morale is the only variable here that multiplies everything and
...
- **Say:** "Urgency is the only management input with no direct production
...
- **Say:** "Competence mismatch both suppresses today's output and mints
...
- **Say:** "Volatility doesn't accrue debt — it doesn't stay pointed at
...
- **Say:** "Debt is not a penalty, it's an interest rate. The slider is where
...
- **On screen:** Raise R: score dips immediately; the debt derivative goes
  ...
  quarter.
- **Say:** "Here's the trap fully armed: every short-horizon measurement in
...
*(The knobs behind the curtain. In-world defense if challenged: these are
...
- **α / β — Competence / Volatility sensitivity:** The Ω weights — how hard
  your particular organization's physics punish mismatch vs. churn. "Every
  org has its own exchange rate between confusion and chaos; these are yours."

---

## The 90-second safe loop (when the thread is lost)
...
## Q&A landmines, pre-defused

- *"Your slider says U=1 but the equation says 1.27?"* — Felt urgency: the
  attenuation state amplifies imposed pressure. The UI labels it, the note
  under the slider explains it, and it's the best talking point in the app.
- *"The debt grid terms — do they sum to the rate?"* — Yes, exactly (to
  display rounding). Invite them to add it up. The κ's are the paper's; the
  inputs are felt/effective quantities weighted by A(t), stated in the note
  above the grid.
- *"Where do the constants come from?"* — "The same place your story-point
  velocity targets come from, but with the derivation shown."
- *"Is this a joke?"* — "The paper is satirical. The structure is not. Three
